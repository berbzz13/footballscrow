  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-8 flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900">Club Dashboard</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
        {[
          ...
          { icon: <Eye size={20} className="text-orange-500" />, label: "Pending Deals", value: deals.filter(d => d.status === "pending").length },
          { icon: <Eye size={20} className="text-green-600" />, label: "Completed", value: deals.filter(d => d.status === "completed").length },
        ].map((s) => (
      ...
      <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6">
            ...
            {deals.length === 0 ? (
              <div className="text-center py-10 text-gray-400">
                ...
                <p>No deals yet. Browse talents and request a deal.</p>
                <Link href="/talents" className="text-green-600 text-sm font-semibold hover:underline mt-2 block">
                  Browse Talents →
                </Link>
              </div>
            ) : (
                  ...
                  <div key={deal.id} className="border border-gray-200 rounded-xl p-4">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        {deal.talent.talentProfile?.position && (
                          <span className="ml-2 text-xs bg-green-50 text-green-700 px-2 py-0.5 rounded">{deal.talent.talentProfile.position}</span>
                        )}
                        {deal.video && (
                          <p className="text-xs text-gray-500 mt-0.5">Re: {deal.video.title}</p>
                        )}
                        <p className="text-xs text-gray-400 mt-0.5">Requested {formatDate(deal.createdAt)}</p>
                      </div>
                  ...
                  return (
                    <div key={interest.id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-gray-50">
                      ...
                      <div className="flex-1 min-w-0">
                        <Link href={`/talent/${talent.id}`} className="font-semibold text-gray-900 hover:text-green-600 text-sm">
                          {talent.name}
                        </Link>
                      ...
                      <Link href={`/videos/${interest.videoId}`} className="text-xs text-green-600 hover:underline shrink-0">
                        View Video →
                      </Link>
                    </div>
                  );
        ...
        <div>
          <div className="bg-white rounded-2xl border border-gray-200 p-6">
            <h2 className="text-lg font-bold text-gray-900 mb-4">Club Profile</h2>
            <ClubProfileForm user={user} />
          </div>
        </div>
      </div>
    </div>
